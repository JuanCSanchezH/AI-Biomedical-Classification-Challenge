...
class DataProcessor:
    ...
    def clean_text(self, text):
        ...
        tokens = [
            self.lemmatizer.lemmatize(token) for token in tokens if token not in self.stop_words and len(token) > 2
        ]
    ...
    def prepare_data(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            self.data["text_combined"], y, test_size=0.2, random_state=42, stratify=None
        )
